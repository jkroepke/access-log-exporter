...
func Benchmark_Syslog_Unix(b *testing.B) {
...
func Benchmark_Syslog_IP(b *testing.B) {
	udpListener, err := nettest.NewLocalPacketListener("udp")
	require.NoError(b, err)
	...
	server, err := syslog.New(b.Context(), slog.New(slog.DiscardHandler), "udp://"+udpListener.LocalAddr().String(), logBuffer)
	require.NoError(b, err)
	...
	syslogClient, err := dial.DialContext(b.Context(), "udp", udpListener.LocalAddr().String())
	require.NoError(b, err)
